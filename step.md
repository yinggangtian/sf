...
**状态：✅ 已完成（9/9 tests passed）**
- 创建了 3 个工具包装类：RAGTool、ProfileTool、HistoryTool（带 JSON Schema）
- 实现了 MasterAgent.run() 完整流程：Orchestrator → 工具调用 → Explainer
- 实现了超时保护（ThreadPoolExecutor + timeout=10s）和降级策略
- 测试覆盖：完整流程、追问、超时、工具失败、RAG/Profile 降级、异常处理
...
**状态：✅ 已完成**
- 创建了 4 个完整的工具描述文档（liuren_tool.md、rag_tool.md、profile_tool.md、history_tool.md）
- 每个文档包含：功能描述、参数说明、返回格式、使用示例、注意事项、JSON Schema
- 补充了特定要求：参数约束（1-6 范围）、超时降级说明、隐私保护声明、分页参数
- 所有工具类都实现了 get_tool_schema() 静态方法，返回符合 OpenAI Function Calling 规范的 Schema
- 枚举类型使用 enum 限制，必填参数在 required 字段中声明
